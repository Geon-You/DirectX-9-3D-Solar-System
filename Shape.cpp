...
void Shape::Update()
{
	...
	// z축 회전
	if (GetAsyncKeyState('Q')) angle.z -= 1.f;
	if (GetAsyncKeyState('E')) angle.z += 1.f;

	// y축 회전
	if (GetAsyncKeyState('W')) angle.y -= 1.f;
	if (GetAsyncKeyState('S')) angle.y += 1.f;

	// x축 회전
	if (GetAsyncKeyState('A')) angle.x -= 1.f;
	if (GetAsyncKeyState('D')) angle.x += 1.f;
...
void Shape::Render()
{
	...
	// -> 광원 옵션은 켜져있지만, 실제 월드에 배치된 광원이 없으므로 광원을 껐음
	...
	// 짐벌락 문제가 발생함
	// 이 짐벌락 문제를 최소화 할 수 있는 것이 쿼터니언
	// 그리고 여러 축에 대한 회전 연산에 대해서는 쿼터니언이 연산속도가 더 빠름
...
bool Shape::OnCollisionEnter(D3DXVECTOR3* ori, D3DXVECTOR3* dir)
{
	...
	// 광선과 삼각형의 충돌체크 -> D3DXIntersectTri();
